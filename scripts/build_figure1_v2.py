...
def build_option_a() -> Path:
    ...
    claim_titles = ["Expression", "Specificity", "Transfer", "Error / direction"]
    claim_states = ["metric-specific", "supported / narrowed", "supported / degraded", "endpoint-specific"]
...
def build_option_b() -> Path:
    ...
    stages = [
        ...
        ("2", "Expression-\nagreement audit", PALETTE["measure_fill"], PALETTE["measure_edge"]),
        ("3", "Perturbation-\nspecific audit", PALETTE["specific_fill"], PALETTE["specific_edge"]),
        ("4", "Falsification\naudit", PALETTE["false_fill"], PALETTE["false_edge"]),
        ("5", "Transfer and\nerror/direction audit", PALETTE["transfer_fill"], PALETTE["transfer_edge"]),
    ]
    x0 = 0.045
    y = 0.585
    w = 0.145
...
def build_docx() -> str:
    ...
    if not md_path.exists():
        return "SKIPPED: manuscript/CRM_MANUSCRIPT_v1.5.md not present"
    doc = Document()
    sec = doc.sections[0]
