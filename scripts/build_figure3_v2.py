...
METHOD_ORDER = ["Mean-effect", "Label-shuffled", "GEARS"]
MODEL_MAP = {
    "Mean-effect": "B1_global_perturbed_mean",
    "Label-shuffled": "FP3_label_shuffled_mean_effect",
    "GEARS": "GEARS_cell_gears_0.1.2",
}
DISPLAY_DEFINITION = {
    "Mean-effect": "target-blind response probe/baseline",
    "Label-shuffled": "target labels scrambled diagnostic probe",
...
METHOD_STYLE = {
    ...
    "Label-shuffled": dict(marker="^", facecolors="white", edgecolors="#8a6f35", linewidths=1.2, s=54),
...
def load_plot_data() -> pd.DataFrame:
    ...
    gears = pd.read_csv(GEARS_SUMMARY)
    rows = []
                ...
                rows.append(
                    {
                        ...
                        "mrr": float(row["mrr"]),
                        "mrr_ci_low": float(row["mrr_ci_low"]),
                        "mrr_ci_high": float(row["mrr_ci_high"]),
...
def candidate_registry(plot_data: pd.DataFrame) -> pd.DataFrame:
    retrieval = pd.read_csv(PROBE_RETRIEVAL)
    rows = []
    for context in CONTEXTS:
            ...
            rows.append(
                {
                    ...
                    "random_mrr_if_valid": random_mrr(n_candidates),
...
def draw_option_a(data: pd.DataFrame, out_stem: Path) -> plt.Figure:
    ...
    labels.append("Random-rank reference: HN/N")
...
def draw_option_b(data: pd.DataFrame, out_stem: Path) -> plt.Figure:
    ...
    offsets = {"Label-shuffled": -0.35, "GEARS": 0.0, "Mean-effect": 0.35}
    for ax, (metric, title, xlim) in zip(axes, metrics):
        ...
        for context in CONTEXTS:
            ...
            xs = []
            ys = []
            for method in ["Label-shuffled", "GEARS", "Mean-effect"]:
...
def write_reports(plot_data: pd.DataFrame, registry: pd.DataFrame) -> None:
    ...
    editor = [
        ...
        "| Which probes lack intact perturbation-specific information? | PASS | Mean-effect and Label-shuffled are formally labeled and defined in the legend. |",
        "| Can response agreement persist in these probes? | PASS | Panel A shows substantial Mean-effect Pearson values and non-zero Label-shuffled values. |",
