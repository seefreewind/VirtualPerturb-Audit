...
except ModuleNotFoundError:  # pragma: no cover - exercised in lightweight release checks.
    torch = None
    optim = None
...
def build_filtered_go_tensors(data_root: Path, dataset_name: str, pert_data) -> tuple[torch.Tensor, torch.Tensor]:
    if torch is None:
        raise ModuleNotFoundError("PyTorch is required for GEARS tensor construction.")
...
def train_smoke_batches(model, max_train_batches: int) -> list[float]:
    if torch is None or optim is None:
        raise ModuleNotFoundError("PyTorch is required for GEARS training.")
...
def evaluate_gears_batches(model, max_eval_batches: int) -> tuple[dict, dict]:
    if torch is None:
        raise ModuleNotFoundError("PyTorch is required for GEARS evaluation.")
    pred_by_pert = {}
    truth_by_pert = {}
...
def append_gears_summary(
    ...
    if shared:
        centroid_payload = {
            ...
            "pred_delta": {pert: pred_deltas[pert] for pert in shared},
            "truth_delta": {pert: truth_deltas[pert] for pert in shared},
        }
        if torch is not None:
            ...
            with (outdir / "gears_delta_centroids.pt").open("wb") as f:
                pickle.dump(centroid_payload, f)
